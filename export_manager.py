...
class ExportManager:
    ...
    def export_escalations_csv(self) -> io.StringIO:
        """Export escalation events as CSV"""
        ...
        for esc in data.get("recent_escalations", []):
            writer.writerow([
                esc.get("case_id", ""),
                esc.get("sentiment_score", ""),
                esc.get("sentiment_label", ""),
                esc.get("escalation_reason", ""),
                esc.get("status", ""),
                str(esc.get("timestamp", ""))[:16].replace("T", " "),
            ])
    ...
    def export_trends_csv(self) -> io.StringIO:
        """Export conversation trends as CSV"""
    ...
    def export_full_pdf(self) -> io.BytesIO:
        ...
        if trends:
            trend_data = [["Date", "Messages"]]
            for t in trends:
                trend_data.append([t.get("date", ""), str(t.get("messages", 0))])
        ...
        if recent_escs:
            ...
            for e in recent_escs:
                esc_data.append([
                    e.get("case_id", ""),
                    str(e.get("sentiment_score", "")),
                    e.get("sentiment_label", "").upper(),
                    e.get("escalation_reason", "")[:40] + ("..." if len(e.get("escalation_reason", "")) > 40 else ""),
                    e.get("status", "").upper(),
                ])
